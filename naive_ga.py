...
def evaluate_network(individual, inp_file, strategy="static", gen=0, tolerance=0.0, total_gens=100):
    ...
    with warnings.catch_warnings():
        ...
        GLOBAL_SIM_COUNT += 1 # РАХУЄМО СИМУЛЯЦІЮ
        try:
            results = sim.run_sim()
        except Exception:
            return 1e15, 
...
def run_single_trial(run_id, args):
    ...
    for gen in range(args.gen):
        ...
        # --- ПЕРЕВІРКА НА СТАГНАЦІЮ ---
        if raw_val < best_cost_so_far - 1.0: # Якщо є покращення хоча б на 1 долар
            best_cost_so_far = raw_val
            stagnation_counter = 0
        else:
        ...
        if gen % 10 == 0 or gen == args.gen - 1:
            try:
                ...
                print(f"    [Run {run_id+1}] Gen {gen:3d}: Cost={cost_disp/1e6:.2f}M$ | P={p_disp:.2f}m {status} | Sims={GLOBAL_SIM_COUNT} | Time={format_time(elapsed)}")
            except: pass

        # --- РАННЯ ЗУПИНКА ---
        if stagnation_counter >= 100:
            print(f"    [Auto-Stop] 🛑 Stagnation limit reached (100 gens without improvement). Stopping run.")
            break

    best_ind = hof[0]
    real_cost, min_p = get_real_stats(best_ind, args.inp)
    run_time = time.time() - run_start
    print(f"    >> Run #{run_id + 1} Done ({format_time(run_time)}). Final: {real_cost/1e6:.4f} M$ | P: {min_p:.2f} m | Sims: {GLOBAL_SIM_COUNT}")
    
    return best_ind, real_cost, min_p, run_time, history_log, GLOBAL_SIM_COUNT
...
if __name__ == "__main__":
    ...
    out_dir = os.path.join("OutputData_Naive", timestamp)
    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(os.path.join(out_dir, "logs"), exist_ok=True)
    os.makedirs(os.path.join(out_dir, "tables"), exist_ok=True)
    os.makedirs(os.path.join(out_dir, "plots"), exist_ok=True)
    
    log_file = os.path.join(out_dir, "logs", f"naive_run_{timestamp}.txt")
    ...
    print("==============================================")
    print(f"   NAIVE EVOLUTIONARY OPTIMIZER: {args.inp}")
    print("==============================================")
    ...
    for i in range(args.runs):
        # Reset sim count for each run
        GLOBAL_SIM_COUNT = 0
        ind, cost, pressure, duration, hist, sims = run_single_trial(i, args)
        ...
        results_summary.append({
            "Run": i + 1, "Cost": cost_pol, "Pressure": pressure_pol,
            "Feasible": is_feasible, "Simulations": GLOBAL_SIM_COUNT, "Time": duration
        })
        
        # Save individual run history
        pd.DataFrame(hist).to_csv(os.path.join(out_dir, "tables", f"run_{i+1}_history.csv"), index=False)

    # Save Summary
    summary_df = pd.DataFrame(results_summary)
    summary_df.to_csv(os.path.join(out_dir, "tables", "runs_summary.csv"), index=False)
    ...
    print("\n\n=== FINAL LEADERBOARD ===")
    print(f"{'Run':<4} {'Cost (M$)':<10} {'Pressure':<10} {'Simulations':<15} {'Time':<10} {'Status':<6}")
    print("-" * 65)
    for res in sorted_results:
        status = "OK" if res['Feasible'] else "FAIL"
        print(f"{res['Run']:<4} {res['Cost']/1e6:<10.4f} {res['Pressure']:<10.3f} {res['Simulations']:<15} {format_time(res['Time']):<10} {status:<6}")
    
    print(f"\nCHAMPION: Run #{best_run['Run']}")
